...
async def generate_cover_letter_text(
    ...
    """Generate a highly personalized cover letter using Mistral. Optionally mimics the style of provided examples."""
    ...
    user_content += "Maintenant, rédige la lettre de motivation finale."
...
async def revise_cover_letter_text(
    ...
    """Take an existing cover letter and user instructions to generate a revised version."""
    settings = get_settings()
    client = Mistral(api_key=settings.mistral_api_key)
    ...
    logger.info("Revising cover letter...")
